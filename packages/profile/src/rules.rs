...
/// 剂型/途径关键词要找的那片干草:**规范名 + 最近一条医嘱的原样写法 + 剂量串**,小写。
///
/// `m.name` 是词典归一之后的规范名,而归一会把剂型吃掉(「地塞米松注射液」→
/// 「地塞米松」)。等效换算表填进包以后,只看规范名就挡不住注射剂了:一次静脉冲击
/// 会被当成口服日剂量,换算出一个几十毫克的「泼尼松等效」并直接进 DORIS/LLDAS。
///
/// ⚠️ **必须用 `latest_raw_name`,不能用 `raw_names`。** 后者是整条 span 的历史并集
/// (一个 drug_key 的所有 mention 合并),而 `latest_dose` 取的是**最近一条**医嘱 ——
/// 两者不同源。用并集会让历史上任何一次冲击**永久**挡住今天的口服剂量,并在卡上把
/// 今天那片 8 mg 口服药标成「注射剂型」;而「冲击 → 口服维持」恰恰是 SLE 最常见的
/// 激素用法,`gc_ca_vitd` / `gc_dxa` 两条强推荐会因此对**激素负荷最重**的那群人静默
/// 消失。剂型是「**这一条医嘱**是口服还是输液」,不是「这个人这辈子用过什么」。
///
...
fn form_haystack(m: &parser::MedSpan) -> String {
    format!(
        "{} {} {}",
        m.name,
        m.latest_raw_name.as_deref().unwrap_or_default(),
