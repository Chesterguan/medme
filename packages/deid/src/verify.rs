...
pub struct Verified { pub extraction: Extraction, pub rejected: usize, pub unverified: usize, pub unverified_fields: Vec<String> }
...
fn comma_between_digits_to_dot(s: &str) -> String {
    ...
    for (i, &c) in chars.iter().enumerate() {
        ...
        out.push(if c == ',' && prev_digit && next_digit { '.' } else { c });
    }
    out
}

/// 去掉所有空白;只用于文本字段的图片档比对——文本字段跨行拼接不构成"假验真"风险。
fn strip_ws(s: &str) -> String { s.split_whitespace().collect() }

/// 文本档:逐字子串,对所有字段一视同仁。
/// 图片档:数值字段只做 `,`→`.` 归一(不去空白,防跨行数字拼接误判);文本字段允许去空白后比对。
/// 空字段视为通过——LLM 没提取到不算错。
fn field_ok(value: &str, src: &str, src_num: &str, src_ws: &str, mode: Mode, kind: FieldKind) -> bool {
...
fn check_top_field(
    ...
    match mode {
        Mode::Text => { field.clear(); *rejected += 1; }
        Mode::Image => { unverified_fields.push(name.to_string()); *unverified += 1; }
    }
}
...
pub fn verify(mut e: Extraction, source_text: &str, mode: Mode) -> Verified {
    ...
    let ok = field_ok(&e.doc_date, source_text, &src_num, &src_ws, mode, FieldKind::Text);
    check_top_field("doc_date", &mut e.doc_date, ok, mode, &mut rejected, &mut unverified, &mut unverified_fields);
    let ok = field_ok(&e.impression, source_text, &src_num, &src_ws, mode, FieldKind::Text);
    check_top_field("impression", &mut e.impression, ok, mode, &mut rejected, &mut unverified, &mut unverified_fields);
    let ok = field_ok(&e.notes, source_text, &src_num, &src_ws, mode, FieldKind::Text);
    check_top_field("notes", &mut e.notes, ok, mode, &mut rejected, &mut unverified, &mut unverified_fields);
    ...
    e.labs.retain_mut(|l| {
        let ok = field_ok(&l.name, source_text, &src_num, &src_ws, mode, FieldKind::Text)
            && field_ok(&l.value, source_text, &src_num, &src_ws, mode, FieldKind::Numeric)
            && field_ok(&l.unit, source_text, &src_num, &src_ws, mode, FieldKind::Text)
            && field_ok(&l.ref_low, source_text, &src_num, &src_ws, mode, FieldKind::Numeric)
            && field_ok(&l.ref_high, source_text, &src_num, &src_ws, mode, FieldKind::Numeric)
            && field_ok(&l.flag, source_text, &src_num, &src_ws, mode, FieldKind::Text);
        keep(ok, &mut l.unverified)
    });
    e.meds.retain_mut(|m| {
        let ok = field_ok(&m.name, source_text, &src_num, &src_ws, mode, FieldKind::Text)
            && field_ok(&m.dose, source_text, &src_num, &src_ws, mode, FieldKind::Text)
            && field_ok(&m.freq, source_text, &src_num, &src_ws, mode, FieldKind::Text)
            && field_ok(&m.route, source_text, &src_num, &src_ws, mode, FieldKind::Text);
        keep(ok, &mut m.unverified)
    });
    e.diagnoses.retain_mut(|d| {
        let ok = field_ok(&d.text, source_text, &src_num, &src_ws, mode, FieldKind::Text)
            && field_ok(&d.icd, source_text, &src_num, &src_ws, mode, FieldKind::Text);
        keep(ok, &mut d.unverified)
    });

    Verified { extraction: e, rejected, unverified, unverified_fields }
}
...
mod tests {
    ...
    fn e() -> Extraction {
        Extraction {
            ...
            diagnoses: vec![DiagnosisItem { text: "2型糖尿病".into(), icd: "E11.9".into(), ..Default::default() }],
            ..Default::default()
        }
    }
    ...
    fn hallucinated_doc_date_is_dropped_in_text_mode() {
        let ex = Extraction { doc_date: "2099-01-01".into(), ..Default::default() };
        let v = verify(ex, SRC, Mode::Text);
    ...
    fn hallucinated_impression_and_notes_are_dropped_in_text_mode() {
        let ex = Extraction { impression: "杜撰的印象".into(), notes: "杜撰的备注".into(), ..Default::default() };
        let v = verify(ex, SRC, Mode::Text);
    ...
    fn fabricated_lab_flag_drops_item_in_text_mode() {
        let ex = Extraction {
            labs: vec![LabItem {
                name: "白细胞计数".into(), value: "5.6".into(), unit: "10^9/L".into(),
                ref_low: "4.0".into(), ref_high: "10.0".into(), flag: "↑".into(),
                ..Default::default()
            }],
            ..Default::default()
        };
    ...
    fn med_with_hallucinated_dose_is_dropped_in_text_mode() {
        let ex = Extraction {
            meds: vec![MedItem { name: "阿司匹林肠溶片".into(), dose: "500mg".into(), ..Default::default() }],
            ..Default::default()
        };
    ...
    fn diagnosis_with_wrong_icd_is_dropped_in_text_mode() {
        let ex = Extraction {
            diagnoses: vec![DiagnosisItem { text: "2型糖尿病".into(), icd: "E11.0".into(), ..Default::default() }],
            ..Default::default()
        };
        let v = verify(ex, SRC, Mode::Text);
    ...
    fn image_mode_does_not_fuse_numbers_across_lines() {
        ...
        let ex = Extraction {
            labs: vec![LabItem { name: "血红蛋白".into(), value: "150160".into(), ..Default::default() }],
            ..Default::default()
        };
        let v = verify(ex, src, Mode::Image);
        assert_eq!(v.extraction.labs.len(), 1);
        assert!(v.extraction.labs[0].unverified, "150 与 160 跨行相邻,不应拼接验真");
        assert_eq!(v.unverified, 1);
    }
    ...
    fn image_mode_number_normalization_still_matches_comma_decimal() {
        ...
        let ex = Extraction {
            labs: vec![LabItem { name: "血糖".into(), value: "7.1".into(), ..Default::default() }],
            ..Default::default()
        };
        let v = verify(ex, src, Mode::Image);
