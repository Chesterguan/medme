...
/// 化验状态 → 前景色。正常(`LabFlag.normal`)与无标记(`null`)都是正文墨色
/// ——减法稿 2026-09-22:颜色只说状态,正常不上色(与 `lab_status.dart` 的
/// `labStatusColor` 同一套规则)。
Color _flagColor(BuildContext context, LabFlag? flag) {
  final c = MedColors.of(context);
  return switch (flag) {
    LabFlag.high => c.high,
    LabFlag.low => c.low,
    LabFlag.normal => c.ink,
    null => c.ink,
  };
}

/// 化验状态 → 右列状态词(减法稿 2026-09-22:上了色的字,无底,`statusWord`,
/// 取代旧的左侧色条 + 文字 pill 双编码)。正常/无标记什么都不给。
///
...
Widget? _flagWord(BuildContext context, LabFlag? flag) {
  final c = MedColors.of(context);
  return switch (flag) {
    LabFlag.high => statusWord('偏高', c.high),
    LabFlag.low => statusWord('偏低', c.low),
    _ => null,
  };
}
...
/// 化验表:一行一项,状态编码在**右列的状态词**上(减法稿 2026-09-22:无色条,
/// 正常不上色不加字,与 `lab_status.dart` 的 `LabLine` 同一条规则)。
///
...
class _LabRowView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final word = _flagWord(context, row.flag);
    return Container(
      decoration: BoxDecoration(
        // 减法稿 2026-09-22:不再画左侧状态色条,只留行间分隔线(bottom)。
        border: Border(
          bottom: last ? BorderSide.none : BorderSide(color: c.line2),
        ),
      ),
      // 左右内边距对齐 s2(12),与表头一致;不再留 4px 色条占位。
      padding: const EdgeInsets.fromLTRB(MedShape.s2, 9, MedShape.s2, 9),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Expanded(
