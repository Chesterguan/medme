...
enum AnalyticsEvent {
  ...
  ///
  /// 回答的决定:**「只看异常」默认关对不对。** Task 6 把这颗开关(原来叫「只看
  /// 非正常项」)的默认值从「开」改成了「关」——合并列表已经把异常序列排到了
  /// 前面,默认再藏一半正常项反而分不清「只测一次」和「趋势」。
  /// [TrendsFilterControl.abnormalOnlyOn] 的频次就是这次改动的检验:开得多 =
  /// 默认关错了、该考虑改回默认开。
  ///
...
enum TrendsFilterControl {
  /// 「只看异常」被关掉。Task 6 起默认就是关——这一档只可能出现在**先打开过**
  /// 之后(默认关着,不存在"从没开过又关一次"这种事件):是 [abnormalOnlyOn]
  /// 的分母,「开过之后又关回去」,单独看没有意义。
  abnormalOnlyOff,

  /// 「只看异常」被主动打开(用户要收窄到异常)。Task 6 把默认值从开改成了关,
  /// 所以**这一档才是信号**——合并列表已经把异常排在前面,用户还愿意伸手去开
  /// 这颗默认关着的开关,说明排序不够、他想要更干净的过滤。开关搜索时整颗隐藏
  /// (`PanelChipsRow`),这两档事件天然只统计未搜索时的浏览。
  abnormalOnlyOn,
