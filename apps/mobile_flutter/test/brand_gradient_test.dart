...
/// WCAG 相对亮度 / 对比度。与 `test/first_run_consent_test.dart` 里那份独立算
/// 一遍——不引用实现,免得实现算错了测试还跟着算错(同一条理由,原样抄一份)。
...
void main() {
  ...
  testWidgets('MedPrimaryButton(启用):药丸、按钮阴影、17·w500 白字', (tester) async {
    // fix round 1(task-14-review R29):`onPressed` 没默认值即 `null`,以前
    // 启用/禁用画得一模一样所以漏了也测不出来;现在两条路径外观不同,这条测的
    // 是启用态,显式传一个回调,不再依赖「默认值恰好长这样」。
    await tester.pumpWidget(MaterialApp(home: Scaffold(
      body: MedPrimaryButton(label: '出码给医生看', icon: Icons.qr_code_2_outlined, onPressed: () {}))));
    final box = tester.widget<Ink>(find.descendant(
      of: find.byType(BrandGradientBox), matching: find.byType(Ink)).first);
    final d = box.decoration! as BoxDecoration;
