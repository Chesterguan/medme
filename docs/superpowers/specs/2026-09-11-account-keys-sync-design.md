...
## 7. 已知缺口(安全)· 2026-09-12 补

### 旧设备扫码批准新设备:**整对替换**挡不住

换机三条路里的「旧设备批准」(§2 那条"无摩擦"路径)现在真的接线了(新设备出一张
`mdv1.<device_id>.<临时公钥>` 的码,旧设备扫它、把账号私钥封给那把临时公钥)。新设备
拿到的私钥会和 `GET /v1/account/keys` 的 `public_key` 做一次**密钥对匹配探针**
(`AccountFlow.unlockWithDeviceApproval`),于是"服务端只把 `public_key` 换成自己那把"
这种无声攻击被挡住了。

**挡不住**的是整对替换:服务端自造一对 `(pub_a, sec_a)`、把 `sec_a` 封给那张码里的
`eph_public`(它是公开的)、同时发下 `pub_a`。那是一对真密钥,探针通过。

这不是实现疏漏,是这条路的形状:码是**单向**的(新手机 → 旧手机),新设备没有任何
经过认证的渠道能知道"我账号真正的公钥是哪一把"。

- **影响**:受害者已有的云档案一个都打不开(它们用真公钥封的),`restoreProfileKeys`
  逐条跳过 → 新手机上看起来像"档案没回来";此后新建的档案密钥都封给 `pub_a`。
  留下可见异常,但不是被密码学挡住的。
- **真正的堵法**:双向确认 —— 两台手机各显示同一把账号公钥的指纹(短认证串),
  让用户对一眼。**没做。**
- 口令 / 恢复码两条路没有这个缺口:那两份密文只有用户知道的秘密才解得开。
