...
# 前提:同目录 env.sh 里 export 了 DATABASE_URL、PNVS_SIGN_NAME、PNVS_TEMPLATE_CODE、VPC_ID、VSWITCH_ID、SG_ID;
# JWT/HMAC 密钥若 env.sh 里没有则现场随机生成,只进函数环境变量,不落盘。
set -euo pipefail
export PATH=/opt/homebrew/bin:$PATH
HERE=$(dirname "$0"); source "$HERE/env.sh"
: "${DATABASE_URL:?}" "${VPC_ID:?}" "${VSWITCH_ID:?}" "${SG_ID:?}"
...
PY
if aliyun --profile medme fc GET /2023-03-30/functions/medme-api >/dev/null 2>&1; then
