...
- **端到端加密分享**(桌面端;手机端已于 2026-07-28 撤掉此入口,改用二维码):AES-256-GCM 加密成一个自包含网页,凭口令在**任意浏览器本地解密**查看,**不经服务器**。分享时可写一个建议复阅期限 —— 它是**给医生的提醒,不是访问控制**:文件在对方手里,过期后仍可打开(我们不假装能撤回)。也可一键导出自包含 HTML 交给医生打印。
- **可核验审计**:不可篡改的审计追踪(导入 / 导出 / 分享均记内容哈希 sha256);**数据安全引导**(FileVault + iCloud 高级数据保护)。

## 安装 · Install

- **手机端(当前主力)**:到 [Releases](https://github.com/Chesterguan/medme/releases)
  或[官网](https://medmenow.com/)下载 `MedMe-Android.apk`(需 arm64 手机);iOS 走 TestFlight。
- **桌面端:暂缓,没有公开安装包。** Releases 和官网上都**没有** `.dmg` —— 要用得自己从源码构建(见下)。
...
- **默认不上传**:病历留在你的设备上,没有账号、没有云端档案。**唯一的例外**是当面出示
  二维码给医生:那一刻密文会临时经过一个瞬时中转(保留期到即删),**密钥只在二维码的
  `#` 之后、从不上行**,所以我们和中转方都解不开。别处必然还有一份 —— 云上那份永远不是
  唯一的副本。
