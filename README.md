...
- **端到端加密分享**:AES-256-GCM 加密成一个自包含网页,凭口令在**任意浏览器本地解密**查看,**不经服务器**。分享时可写一个建议复阅期限 —— 它是**给医生的提醒,不是访问控制**:文件在对方手里,过期后仍可打开(我们不假装能撤回)。也可一键导出自包含 HTML 交给医生打印。
...
- **推荐(无需开发环境)**:到 [Releases](https://github.com/Chesterguan/medme/releases)
  下载 `.dmg`(Apple Silicon / Intel),拖进「应用程序」即可。
  ⚠️ **桌面端目前暂缓、官网上没有公开安装包**(见落地页「即将推出」);
  手机端是当前主力,官网可下载安卓 APK。
